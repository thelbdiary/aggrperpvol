  ...
  async getHistoricalVolume(startTime, endTime) {
    try {
      ...
      try {
        ...
        // Log API key format for debugging (only first few characters)
        if (this.apiKey.length > 5) {
          console.log(`WooX API key format: ${this.apiKey.substring(0, 5)}...`);
        }
        
        // Check if API key is in Base64 format (common format for WooX keys)
        const isBase64 = /^[A-Za-z0-9+/=]+$/.test(this.apiKey);
        if (!isBase64) {
          console.warn('WooX API key does not appear to be in Base64 format, this may cause authentication issues');
        }
        
        // Check if API secret is in expected format
        if (this.apiSecret.length < 10) {
          console.warn('WooX API secret appears to be too short, this may cause authentication issues');
        }
